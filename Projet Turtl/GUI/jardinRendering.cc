...
void JardinRendering::construction(std::string file){
    if (file.compare("")){
        ...
        if(fichier) {    
            ...
            changeTailleJardin(width,height);
...
void JardinRendering::changeCouleur(int numeroTortue, int r, int g, int b){
    tortues.at(numeroTortue)->setCouleurCarapace(QColor(r,g,b));
    EtatTortue E = EtatTortue(numeroTortue,*tortues.at(numeroTortue));
    E.setCouleur(QColor(r,g,b));
    mouvements.push(E);
}
/*DEPRECATED
...
void JardinRendering::supprimerTortue(int numeroTortue){
     delete tortues[numeroTortue];
     tortues.erase(tortues.begin()+numeroTortue-1);
     emit deleteTortue(numeroTortue);
}
