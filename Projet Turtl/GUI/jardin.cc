...
:QWidget(parent){
    ...
    connect(render, SIGNAL(deleteTortue(int)), this, SLOT(supprimerTortue(int)));
 //   connect(render, &JardinHandler::parsingFinished, this, &MainWindow::onParsingFinished);
...
void Jardin::supprimerTortue(int numeroTortue){
    tortues.erase(tortues.begin()+numeroTortue-1);
    repaint();
}
...
void Jardin::updateTortue(EtatTortue mouvement){
    ...
    tortues.at(mouvement.getNumeroTortue()).setCouleurCarapace(mouvement.getCouleur());
